...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        dfk = p_scores
        dfk[range(amount), y] -= 1
        dfk /= amount
        dW2 = (dfk.T).dot(h1)
        db2 = np.sum(dfk, axis=0, keepdims=True)
        db2 = db2[0,:]
        dh1 = W2.dot(dfk.T)
        dW1 = dh1
        dW1[ h1_scores.T <= 0 ] = 0
        ...
        grads = {'W1': dW1.T + reg*W1, 'W2': dW2.T + reg*W2, 'b1': db1, 'b2': db2}
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            ####################################################################
            selection = np.random.choice(range(y.shape[0]), min(y.shape[0], batch_size), replace=False)
            X_batch = X[selection]
            y_batch = y[selection]
...
def get_accuracy(network, X_train, y_train, X_val, y_val):
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    for rate in learning_rates:
        for reggae in regularization_strengths:
            ...
            print('lr %e reg %e hidden_size %s' % (rate, reggae, hidden_size))
            network.train(X_train, y_train, X_val, y_val,
                    num_iters=2500, batch_size = 300,
                    learning_rate=rate, learning_rate_decay=0.95, reg=reggae, verbose=True)
            train_acc, val_acc = get_accuracy(network, X_train, y_train, X_val, y_val)
            results[(rate, reggae, hidden_size)] = (train_acc, val_acc)
            
            all_classifiers.append(network)
            if best_val < val_acc:
                best_val = val_acc
    ...
    for (lr, reg, hidden_size) in sorted(results):
        ...
        print('lr %e reg %e train accuracy: %f val accuracy: %f hidden_size: %s' % (
              lr, reg, train_accuracy, val_accuracy, hidden_size))
